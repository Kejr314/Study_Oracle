...
SET SERVEROUTPUT ON; -- DBMS_OUTPUT.PUT_LINE 출력됨

DECLARE
    -- EMPLOYEE 테이블의 행 유형을 가지는 ENIFO 변수를 선언
    ENIFO EMPLOYEE%ROWTYPE;
BEGIN
    -- EMPLOYEE 테이블에서 EMP_ID가 사용자 입력 값인 EMPID와 일치하는 행을 ENIFO 변수에 선택
    SELECT * INTO ENIFO FROM EMPLOYEE
    WHERE EMP_ID = '&EMPID';
    
    -- 선택된 직원의 이름과 급여를 출력
    DBMS_OUTPUT.PUT_LINE('이름 : ' || ENIFO.EMP_NAME || ', 급여 : ' || ENIFO.SALARY);
END;
/

-- PL/SQL의 IF문
-- IF (조건식) THEN 실행문 END IF;
-- IF (조건식) THEN 실행문1 ELSE 실행문2 END IF;
-- IF (조건식) THEN 실행문1 ELSE IF (조건식2) THEN 실행문 ELSE 실행문3 ... END IF;
...
DECLARE
    ...
    SLEVEL VARCHAR2(3);
BEGIN
    -- EMPLOYEE 테이블에서 EMP_ID가 사용자 입력 값과 일치하는 행의 EMP_ID, EMP_NAME, SALARY 컬럼 값을 각각 EMPID, EMPNAME, SALARY 변수에 저장
    SELECT EMP_ID, EMP_NAME, SALARY
    INTO EMPID, EMPNAME, SALARY
    FROM EMPLOYEE
    WHERE EMP_ID = '&사번';

    -- SALARY를 100000으로 나눈 값의 소수 부분을 버림
    ...
    DBMS_OUTPUT.PUT_LINE('급여 : ' || SALARY*100000);
...
BEGIN
    LOOP
        ...
        N := N + 1;
        EXIT WHEN N > 5; -- N이 5를 초과하면 루프 종료
    END LOOP; -- 루프 종료
END;
/
...
BEGIN
    LOOP
        DEMS_OUTPUT.PUT_LINE(FLOOR(DBMS_RANDOM.VALUE(1, 11));
        N := N + 1;
        EXIT WHEN N > 5;
    END LOOP;
END;
...
-- 2. WHILE LOOP
-- 제어 조건이 TRUE인 동안만 무장이 반복 실행됨
-- LOOP를 실행할 때 족너이 처음부터 FALSE이면 한번도 수행되지 않을 수 있음
...
BEGIN
    ...
    LOOP
        N := N + 1;
        CONTINUE WHEN MOD(N, 2) = 0; -- 나머지 = 0 -> 짝수 >> 출력 X, 반복
        DBMS_OUTPUT.PUT_LINE(N);
    END LOOP;
END;
/
...
-- 3. FOR LOOP
-- FOR LOOP문에서 카운트용 변수는 자동선언됨, 따로 선언할 필요 없음
-- 카운트 값은 자동으로 1씩 증가
BEGIN
    FOR D IN 1..100
    LOOP
...
BEGIN
    FOR I IN 200..210
    LOOP
        SELECT * INTO EINFO FROM EMPLOYEE WHERE EMP_ID = I;
...
-- PL/SQL 예외처리
-- 시스템 오류(메모리 초과, 인덱스 중복 키 등)는 오라클이 정의하는 에러로 보통 PL/SQL 실행 엔진이 오류 조건을 탐지하여 발생시키는 예외
/*
 - NO_DATA_FOUND
